...
- 商品管理：SKU、图片、名称、多个规格、单位、低库存阈值、备注
- 规格组合：主规格和子规格自动生成组合，例如蓝色-M、蓝色-L、红色-M、红色-L
- 独立库存：每个规格组合单独统计当前库存、累计进货和累计发货
...
1. 新品到货：点“新品入库”，系统会自动生成 SKU；选择文件或直接粘贴商品图片，填写主规格和子规格的名称及值。
2. 初始库存：规格值填写完成后，系统会生成所有组合，分别填写蓝色-M、蓝色-L 等组合的初始数量，再点“保存并入库”。
...
库存数据保存在项目目录的 `data/inventory.db`，商品图片保存在 `data/uploads`。默认单张图片最大 8MB。备份时复制整个 `data` 目录即可。
